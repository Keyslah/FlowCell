...
- **Save Profile** splits the save into two locations:
  - `flowcellbackend/local/Folder Trees/<profile name>/` — a clean, empty mirror of the project's folder structure only (no `.flowcell`, no per-role folders), so it can be copied/pasted manually.
  - `flowcellbackend/local/Folder Tree Profiles/<profile name>.json` — the profile data (roles, file-type assignments, etc.), kept separate from the folder structure.
