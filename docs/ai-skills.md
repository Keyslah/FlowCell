...
- One stable native window per owner toggles between the exact owner footprint and exact saved expanded bounds. The Button Editor's one Pop-out choice and Fan checkbox edit that same surface: Fan mode reveals and focuses the exact freely positioned owner anchor with every child, while Pop mode hides the retained owner and shows every child.
...
Main-page script, macro, and Tool Set owner hosts separate gestures: an ordinary click toggles the exact multi-selection, dispatches no authored runtime event or activation effect, and latches the selected Button into the skin's authored pressed state until deselection; a double-click dispatches that Button's primary action directly on Main. `Fan (N)` may use the current selection. Main `Open Pop` opens one chosen Pop-out file transiently. `Pop` reopens that panel's last-used file or creates/reuses the canonical starter Pop when none is remembered; neither control depends on Main selection. Their mounted runtime hosts retain ordinary single-click execution.
