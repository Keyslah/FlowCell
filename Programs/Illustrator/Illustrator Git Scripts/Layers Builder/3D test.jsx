...
// Runs the original Layers script "3D test" against the
...
/*
 * Saves exactly three selected Live targets into 3D > [name] > dN, runs
 * 3p5mm on the top target and p5mm on the other two, then hides the
 * matching Live targets and keeps the new 3D entries visible and editable.
 */
(function () {
    var SCRIPT_VERSION = "2026-03-27 3D test macro routing";
    var LOG_PATH = Folder.temp.fsName + "/Illustrator_Save_3D_Test_Debug.log";
    ...
    var MACRO_POLL_MS = 250;
    var MACRO_PREP_SETTLE_MS = 1200;
    var MACRO_POST_SETTLE_MS = 1800;
    ...
    try {
        ...
        for (i = 0; i < targets.length; i += 1) {
            ...
            markedCopiedLayers.push({
                ...
                sourceTarget: target,
                macroPlan: getMacroPlanForTargetIndex(i)
            });
        }

        for (i = 0; i < markedCopiedLayers.length; i += 1) {
            var markedTarget = markedCopiedLayers[i];

            runMacroPlanOnMarker(doc, threeDEntry, markedTarget.marker, markedTarget.macroPlan);
            logLine("Processed copied target: " + getTargetName(markedTarget.sourceTarget) + " via " + markedTarget.macroPlan.label);
        }

        hideLiveTargetLayer(branchLayer);
        hidePrevious3DVersions(threeDContainer, threeDEntry);
        activatedLayer = threeDEntry;
        logLine("Saved 3D test branch: " + branchName + " -> " + threeDEntry.name);

    } catch (err) {
        logLine("Exception: " + err);
        alert("3D test failed: " + (err && err.message ? err.message : err));
    } finally {
