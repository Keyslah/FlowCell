...
// Runs the original Layers script "restore" against the
...
(function () {
    ...
    function resolveTargets() {
        ...
        for (i = 0; i < FLOWCELL_LB_TARGETS.layers.length; i += 1) {
            if (layerHasPageItems(FLOWCELL_LB_TARGETS.layers[i])) {
                addResolvedTarget(result, FLOWCELL_LB_TARGETS.layers[i]);
            }
        }
    ...
    function layerHasPageItems(layer) {
        var i;
        ...
        for (i = 0; i < layer.layers.length; i += 1) {
            if (layerHasPageItems(layer.layers[i])) {
                return true;
            }
        }
