...
def _get_workspace_runtime_paths(workspace_root: Path) -> dict[str, Path]:
    ...
    return {
        ...
        "frontend_exe": (frontend_root / "src-tauri" / "target" / "release" / "flowcell_frontend.exe").resolve(),
        "frontend_release_exe": (frontend_root / "src-tauri" / "target" / "release" / "flowcell_frontend.exe").resolve(),
        "frontend_debug_exe": (frontend_root / "src-tauri" / "target" / "debug" / "flowcell_frontend.exe").resolve(),
    }
...
def _get_workspace_process_ids(workspace_root: Path) -> list[int]:
    ...
    frontend_release_exe = str(runtime_paths["frontend_release_exe"])
    frontend_debug_exe = str(runtime_paths["frontend_debug_exe"])
    ps_command = (
        "$frontendReleaseExe='{0}';"
        "$frontendDebugExe='{1}';"
        "$frontendScript='{2}';"
        "$ui='{3}';"
        "$backend='{4}';"
        ...
        "([System.StringComparer]::OrdinalIgnoreCase.Equals($exe, $frontendReleaseExe) -or "
        "[System.StringComparer]::OrdinalIgnoreCase.Equals($exe, $frontendDebugExe)))"
        ") "
        "}} | Select-Object -ExpandProperty ProcessId"
    ).format(
        frontend_release_exe.replace("'", "''"),
        frontend_debug_exe.replace("'", "''"),
...
def _restart_workspace_desktop(target_key: str, workspace_root: Path) -> str:
    ...
    frontend_launcher = runtime_paths["frontend_launcher_script"]
    if frontend_launcher.is_file():
        subprocess.Popen(
            [
                "powershell.exe",
                "-NoProfile",
        ...
        return f"Restarted {target_name} through the release frontend launcher."
