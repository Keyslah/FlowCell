...
def _load_flowcell_bridge():
    ...
    raise RuntimeError("FlowCell Blender bridge module was not found. Reload the FlowCell add-on or restart Blender.") from first_error
...
def _request_flowcell_launch(exported_paths, export_message=""):
    message = export_message or "Saved STL files."
    return {
        "message": f"{message} Choose a slicer EXE in FlowCell to launch the exported STL files.",
...
def run_flowcell_action(context=None, data=None):
    ...
    return _request_flowcell_launch(exported_paths, export_message)
