...
class OBJECT_PT_quick_boolean(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        split.prop(s, "qb_solver", text="Solver")

        row = layout.row(align=True)
        row.prop(s, "qb_self_intersection", text="SI")
        row.prop(s, "qb_hole_tolerant", text="HT")
        row.prop(s, "qb_hide_cutter", text="HC")
        row.prop(s, "qb_backup_active", text="BA")
        row.operator("object.qb_run_auto", text="Run", icon='MOD_BOOLEAN')
