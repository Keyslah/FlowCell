...
| `Cycle Collection` | Uses the selected object's deepest collection, shows one direct object in that collection at a time, and selects the object that is currently visible. Its source script declares `hoverEnter`/`hoverLeave` FlowCell events so hover saves the current visibility and mouse-leave restores it without using the manual visibility baseline. |
