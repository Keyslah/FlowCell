...
# what "Slicer" means lives here.
SLICER_DISPLAY_NAME = "Slicer"
SLICER_EXECUTABLE_LABEL = "Slicer EXE"
SLICER_SETTINGS_FILE_NAME = "slicer_launcher.json"
...
# Generic launcher: there is no well-known install path to probe, so the
# executable is always chosen once and then remembered.
...
def _ensure_orca_settings_confirmed(executable: Path) -> None:
    if not _is_orca_executable(executable):
        return
...
def run_flowcell_action(context=None, data=None):
    ...
    if command == "get_settings":
        ...
        if executable is None:
            raise ValueError("Slicer EXE selection was canceled.")
        is_orca = _is_orca_executable(executable)
        settings = _read_settings()
        result = {
            "message": "Loaded slicer settings.",
        ...
        if is_orca:
            result.update(
                {
                    "allowSingleInstance": _read_orca_single_instance(),
    ...
    if command == "set_single_instance":
        executable = _read_saved_executable()
        if executable is None or not _is_orca_executable(executable):
            raise ValueError("Allow only one OrcaSlicer instance is available only when Orca is selected.")
    ...
    executable = _resolve_executable()
    if executable is None:
        raise ValueError("Slicer EXE selection was canceled, so no file was exported or launched.")
