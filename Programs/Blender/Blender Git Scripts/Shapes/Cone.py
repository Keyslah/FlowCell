...
def perform_boolsafe_cone(context=None, data=None):
    ctx = _ctx(context)
    base_diameter_mm = float(_data_value(data, "base_diameter_mm", DEFAULT_BASE_DIAMETER_MM))
    height_mm = float(_data_value(data, "height_mm", DEFAULT_HEIGHT_MM))
    verts = int(_data_value(data, "verts", DEFAULT_VERTS))

    if base_diameter_mm <= 0.0 or height_mm <= 0.0:
        raise ValueError("Cone base_diameter_mm and height_mm must be greater than zero.")
    if verts < 3:
        raise ValueError("Cone verts must be at least 3.")

    ensure_object_mode(ctx)
    bpy.ops.mesh.primitive_cone_add(
        vertices=verts,
        radius1=mm_to_bu(base_diameter_mm * 0.5, ctx),
        radius2=0.0,
        depth=mm_to_bu(height_mm, ctx),
    ...
    if obj is None:
        return _result("CANCELLED", "Blender did not create a cone object.")
