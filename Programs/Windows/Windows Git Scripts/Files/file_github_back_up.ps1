...
function Get-FlowCellExternalBackupItems([string]$RepositoryRoot, [string]$BackupDateLabel) {
    ...
    $blenderBundleRoot = 'Blender\Blender Git Scripts\blender addons{0}' -f $BackupDateLabel
    $items = @()
    $candidates = @(
        [pscustomobject]@{
        ...
        [pscustomobject]@{
            ...
            Relative = $blenderBundleRoot + '\blender_bridge_flowcell\' + [string]$automation.customActionsFileName
            Label = 'Blender custom action registry'
        },
        [pscustomobject]@{
            Path = Join-Path $bridgeFolder ([string]$automation.setupStatusFileName)
            Relative = $blenderBundleRoot + '\blender_bridge_flowcell\' + [string]$automation.setupStatusFileName
            Label = 'Blender bridge setup state'
        },
        [pscustomobject]@{
            Path = Join-Path $bridgeFolder 'flowcell_bridge_runtime_status.json'
            Relative = $blenderBundleRoot + '\blender_bridge_flowcell\flowcell_bridge_runtime_status.json'
...
function Get-FlowCellObjectPropertyValue([object]$InputObject, [string]$Name) {
...
function Get-FlowCellBlenderConfig([string]$RepositoryRoot) {
    ...
    foreach ($configPath in @($configPaths)) {
        ...
        try {
            ...
            throw ('Could not read Blender config: {0}' -f $configPath)
        }
    }
...
function Get-FlowCellBlenderAutomationValue([object]$Config, [string]$Name, [string]$DefaultValue) {
    if ($null -eq $Config -or $null -eq $Config.automation) {
        return $DefaultValue
    }

    $value = Get-FlowCellObjectPropertyValue -InputObject $Config.automation -Name $Name
    $text = [string]$value
    if ([string]::IsNullOrWhiteSpace($text)) {
        return $DefaultValue
    }
...
function Get-FlowCellBlenderBridgeFolderName([object]$Config) {
    $configuredBridgeFolder = Get-FlowCellBlenderAutomationValue -Config $Config -Name 'bridgeFolder' -DefaultValue ''
    if (-not [string]::IsNullOrWhiteSpace($configuredBridgeFolder)) {
        $leaf = Split-Path -Path $configuredBridgeFolder -Leaf
    ...
    $signals = @(
        (Get-FlowCellBlenderAutomationValue -Config $Config -Name 'customActionsFileName' -DefaultValue ''),
        (Get-FlowCellBlenderAutomationValue -Config $Config -Name 'addonActionsFileName' -DefaultValue ''),
        (Get-FlowCellBlenderAutomationValue -Config $Config -Name 'addonBridgeFileName' -DefaultValue ''),
        (Get-FlowCellBlenderAutomationValue -Config $Config -Name 'addonDisplayName' -DefaultValue '')
    ) -join ' '
    $signals = $signals.ToLowerInvariant()
...
function Resolve-FlowCellBlenderLiveBridgeFolder([object]$Config) {
    $configuredBridgeFolder = Get-FlowCellBlenderAutomationValue -Config $Config -Name 'bridgeFolder' -DefaultValue ''
    if (-not [string]::IsNullOrWhiteSpace($configuredBridgeFolder)) {
        try {
            $resolved = Get-NormalizedFullPath -Path $configuredBridgeFolder
    ...
    $blenderRoot = Join-Path $appDataRoot 'Blender Foundation\Blender'
    if (-not (Test-Path -LiteralPath $blenderRoot -PathType Container)) {
        return ''
    }

    $bridgeFolderName = Get-FlowCellBlenderBridgeFolderName -Config $Config
    $versionDirectories = @(Get-ChildItem -LiteralPath $blenderRoot -Directory -ErrorAction SilentlyContinue | Sort-Object Name -Descending)
    foreach ($versionDirectory in @($versionDirectories)) {
        $candidate = Join-Path $versionDirectory.FullName ('scripts\addons\{0}' -f $bridgeFolderName)
        if (Test-Path -LiteralPath $candidate -PathType Container) {
            return (Get-NormalizedFullPath -Path $candidate)
        }
    }
...
function Copy-FlowCellFirstExistingFile([string[]]$CandidatePaths, [string]$DestinationPath, [string]$Label) {
    ...
    throw ('Could not find {0} for the Blender copy-paste bundle.' -f $Label)
}

function Copy-FlowCellBlenderManagedActionSource([string]$SourcePath, [string]$ManagedDestinationRoot) {
    if ([string]::IsNullOrWhiteSpace($SourcePath)) {
        return ''
    }

    try {
        $resolvedSourcePath = Get-NormalizedFullPath -Path $SourcePath
    }
    catch {
        return ''
    }
    ...
    $leafName = Split-Path -Leaf $resolvedSourcePath
    if ([string]::IsNullOrWhiteSpace($leafName)) {
        return ''
    }

    New-Item -ItemType Directory -Path $ManagedDestinationRoot -Force | Out-Null
    $destinationPath = Join-Path $ManagedDestinationRoot $leafName
    Copy-Item -LiteralPath $resolvedSourcePath -Destination $destinationPath -Force
    return $leafName
}

function Resolve-FlowCellBlenderActionSourcePath([string]$RepositoryRoot, [object]$Entry, [string]$ActionName, [string]$AddonActionsFileName, [string]$AddonBridgeFileName) {
    $candidateTexts = New-Object 'System.Collections.Generic.List[string]'
    foreach ($propertyName in @('sourcePythonPath', 'pythonPath')) {
        $candidateText = [string](Get-FlowCellObjectPropertyValue -InputObject $Entry -Name $propertyName)
        if (-not [string]::IsNullOrWhiteSpace($candidateText)) {
            [void]$candidateTexts.Add($candidateText)
        }
    }

    if (-not [string]::IsNullOrWhiteSpace($ActionName)) {
        [void]$candidateTexts.Add((Join-Path (Join-Path $RepositoryRoot 'Programs\Blender\ManagedActions') ($ActionName + '.py')))
    }

    foreach ($candidateText in @($candidateTexts.ToArray())) {
        ...
        try {
            $normalizedPath = Get-NormalizedFullPath -Path $candidatePath
        }
        catch {
            continue
        }
        ...
        if ($leafName -ieq $AddonActionsFileName -or $leafName -ieq $AddonBridgeFileName) {
            continue
        }
...
function New-FlowCellBlenderRegistryEntriesFromConfig([object]$Config) {
    $entries = @()
    if ($null -eq $Config -or $null -eq $Config.buttons) {
        return @()
    }

    foreach ($button in @($Config.buttons)) {
        $actionName = [string](Get-FlowCellObjectPropertyValue -InputObject $button -Name 'action')
        if ([string]::IsNullOrWhiteSpace($actionName) -or -not $actionName.StartsWith('flowcell_custom_', [System.StringComparison]::OrdinalIgnoreCase)) {
            continue
        }

        $description = [string](Get-FlowCellObjectPropertyValue -InputObject $button -Name 'tooltip')
        if ([string]::IsNullOrWhiteSpace($description)) {
            $description = [string](Get-FlowCellObjectPropertyValue -InputObject $button -Name 'label')
        }

        $entries += [pscustomobject][ordered]@{
            action      = $actionName.Trim().ToLowerInvariant()
...
function Copy-FlowCellBlenderRegistryBundle(
    [string]$RepositoryRoot,
    [object]$Config,
    [string]$LiveBridgeFolder,
    [string]$BundleBridgeRoot,
    [string]$ManagedDestinationRoot,
    [string]$BridgeFolderName,
    [string]$CustomActionsFileName,
    [string]$AddonActionsFileName,
    [string]$AddonBridgeFileName
) {
    $registryEntries = @()
    if (-not [string]::IsNullOrWhiteSpace($LiveBridgeFolder)) {
        $liveRegistryPath = Join-Path $LiveBridgeFolder $CustomActionsFileName
        if (Test-Path -LiteralPath $liveRegistryPath -PathType Leaf) {
            try {
                $liveRegistry = Get-Content -LiteralPath $liveRegistryPath -Raw | ConvertFrom-Json -ErrorAction Stop
                if ($null -ne $liveRegistry.actions) {
                    $registryEntries += @($liveRegistry.actions)
                }
            }
            catch {
            }
    ...
    $registryEntries += @(New-FlowCellBlenderRegistryEntriesFromConfig -Config $Config)
    ...
    foreach ($entry in @($registryEntries)) {
        ...
        if ([string]::IsNullOrWhiteSpace($actionName) -or $seenActions.ContainsKey($actionName)) {
            continue
        }

        $sourcePath = Resolve-FlowCellBlenderActionSourcePath -RepositoryRoot $RepositoryRoot -Entry $entry -ActionName $actionName -AddonActionsFileName $AddonActionsFileName -AddonBridgeFileName $AddonBridgeFileName
        $copiedLeafName = Copy-FlowCellBlenderManagedActionSource -SourcePath $sourcePath -ManagedDestinationRoot $ManagedDestinationRoot
        if ([string]::IsNullOrWhiteSpace($copiedLeafName)) {
            continue
        }

        $entryMap = [ordered]@{}
        foreach ($property in @($entry.PSObject.Properties)) {
            if (@('action', 'pythonPath', 'functionName', 'sourcePythonPath', 'sourceFunctionName', 'startLine') -contains [string]$property.Name) {
                continue
            }
            $entryMap[[string]$property.Name] = $property.Value
        }

        $relativePythonPath = ('{0}/ManagedActions/{1}' -f $BridgeFolderName, $copiedLeafName)
        $functionName = [string](Get-FlowCellObjectPropertyValue -InputObject $entry -Name 'sourceFunctionName')
        $entryMap['action'] = $actionName
        $entryMap['pythonPath'] = $relativePythonPath
        $entryMap['functionName'] = $functionName
        $entryMap['sourcePythonPath'] = $relativePythonPath
        $entryMap['sourceFunctionName'] = $functionName

        [void]$normalizedEntries.Add([pscustomobject]$entryMap)
        $seenActions[$actionName] = $true
    }

    $registryPath = Join-Path $BundleBridgeRoot $CustomActionsFileName
    $registryPayload = [pscustomobject]@{
        actions = @($normalizedEntries.ToArray())
    }
    Set-Content -LiteralPath $registryPath -Value ($registryPayload | ConvertTo-Json -Depth 8) -Encoding UTF8
    return $registryPath
}

function Write-FlowCellBlenderAddonBundleReadme([string]$BundleRoot) {
    $readmePath = Join-Path $BundleRoot 'README.txt'
    $readme = @(
        'FlowCell Blender Add-ons Backup',
        '',
        'Copy these three items into your Blender add-ons folder:',
        '',
        '  flowcell_actions.py',
        '  flowcell_bridge.py',
        ...
        '',
        'Then restart Blender. Enable the add-on named "FlowCell" if Blender has not enabled it yet.'
    ) -join [Environment]::NewLine
    Set-Content -LiteralPath $readmePath -Value $readme -Encoding UTF8
}

function Copy-FlowCellBlenderAddonBundle([string]$DestinationRoot, [string]$RepositoryRoot) {
    $config = Get-FlowCellBlenderConfig -RepositoryRoot $RepositoryRoot
    $addonActionsFileName = Get-FlowCellBlenderAutomationValue -Config $config -Name 'addonActionsFileName' -DefaultValue 'flowcell_actions.py'
    $addonBridgeFileName = Get-FlowCellBlenderAutomationValue -Config $config -Name 'addonBridgeFileName' -DefaultValue 'flowcell_bridge.py'
    $customActionsFileName = Get-FlowCellBlenderAutomationValue -Config $config -Name 'customActionsFileName' -DefaultValue 'flowcell_custom_actions.json'
    $bridgeFolderName = Get-FlowCellBlenderBridgeFolderName -Config $config
    ...
    ) -DestinationPath (Join-Path $bundleRoot $addonActionsFileName) -Label 'Blender add-on actions file')
    [void](Copy-FlowCellFirstExistingFile -CandidatePaths @(
        (Join-Path $repoAddonRoot $addonBridgeFileName),
        $(if (-not [string]::IsNullOrWhiteSpace($liveAddonRoot)) { Join-Path $liveAddonRoot $addonBridgeFileName } else { '' })
    ) -DestinationPath (Join-Path $bundleRoot $addonBridgeFileName) -Label 'Blender add-on bridge file')

    $repoManagedRoot = Join-Path $RepositoryRoot 'Programs\Blender\ManagedActions'
    if (Test-Path -LiteralPath $repoManagedRoot -PathType Container) {
        foreach ($managedAction in @(Get-ChildItem -LiteralPath $repoManagedRoot -Filter '*.py' -File -ErrorAction SilentlyContinue)) {
            Copy-Item -LiteralPath $managedAction.FullName -Destination (Join-Path $managedDestinationRoot $managedAction.Name) -Force
        }
    }

    [void](Copy-FlowCellBlenderRegistryBundle -RepositoryRoot $RepositoryRoot -Config $config -LiveBridgeFolder $liveBridgeFolder -BundleBridgeRoot $bundleBridgeRoot -ManagedDestinationRoot $managedDestinationRoot -BridgeFolderName $bridgeFolderName -CustomActionsFileName $customActionsFileName -AddonActionsFileName $addonActionsFileName -AddonBridgeFileName $addonBridgeFileName)
    Write-FlowCellBlenderAddonBundleReadme -BundleRoot $bundleRoot
...
function Get-CompletionNotificationText([object[]]$Results, [bool]$HasIssues) {
    ...
    if (@($Results).Count -eq 1) {
        ...
        if ($BackupFlavor -eq 'FlowCell' -and @($result.ExternalBackupPaths).Count -gt 0) {
            $lines += ('Extras: {0} item(s)' -f @($result.ExternalBackupPaths).Count)
        }
    }
...
try {
    ...
    if (@($requestedFolders).Count -eq 0) {
        $statusMessage = ('{0} failed. No valid clipboard path found. Copy a folder path to the clipboard first.' -f $script:BackupUiLabel)
        Write-Status $statusMessage
    ...
    if (@($sourceRecords).Count -eq 0) {
        $statusMessage = 'No valid folders were found in the clipboard.'
        Write-Status $statusMessage
        Show-WindowsNotification -Message $statusMessage -Title ($script:BackupUiTitle + ' Failed') -Icon ([System.Windows.Forms.MessageBoxIcon]::Warning)
        exit 1
    }
    ...
    foreach ($sourceRecord in $sourceRecords) {
        ...
        if ($BackupFlavor -eq 'FlowCell' -and $sourceRecord.IsGitRepo) {
            try {
                $externalBackupPaths += (Copy-FlowCellBlenderAddonBundle -DestinationRoot $destinationPath -RepositoryRoot ([string]$sourceRecord.RepositoryRoot))
            }
            catch {
                $externalBackupErrors += $_.Exception.Message
                $failures += ('Blender copy-paste add-ons bundle failed for {0}: {1}' -f $sourcePath, $_.Exception.Message)
            }
        }
    ...
    foreach ($result in $results) {
        ...
        if ($BackupFlavor -eq 'FlowCell') {
            if (@($result.ExternalBackupPaths).Count -gt 0) {
                $summaryLines += ('External support items: {0} copied' -f @($result.ExternalBackupPaths).Count)
            ...
            elseif (@($result.ExternalBackupErrors).Count -gt 0) {
                $summaryLines += 'External support items: incomplete'
                foreach ($externalBackupError in @($result.ExternalBackupErrors)) {
                    $summaryLines += ('External: {0}' -f [string]$externalBackupError)
                }
            }
            else {
                $summaryLines += 'External support items: none detected for this source'
            }
        }
    }
