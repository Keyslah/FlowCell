...
' settings file next to this button. Each button keeps its own choice, so you can
' have several buttons that each toggle to a different monitor. Delete the
' settings file (next to the button) to make that button ask again.
...
' Run the always-current tracked engine, not a stale toggle_monitors.py that may
' sit beside a copied button. "Add Script" only copies this .vbs, so the engine
...
If scriptPath = "" Then
    MsgBox "Toggle Monitors could not find toggle_monitors.py." & vbCrLf & _
           "Re-add the button from Windows Git Scripts \ Utility.", vbExclamation, "Toggle Monitors"
    WScript.Quit 1
End If
