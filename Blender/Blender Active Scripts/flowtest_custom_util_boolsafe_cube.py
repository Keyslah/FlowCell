...
def perform_boolsafe_cube(context=None, data=None):
    ctx = _ctx(context)
    width_mm = float(_data_value(data, "width_mm", DEFAULT_WIDTH_MM))
    depth_mm = float(_data_value(data, "depth_mm", DEFAULT_DEPTH_MM))
    height_mm = float(_data_value(data, "height_mm", DEFAULT_HEIGHT_MM))

    if width_mm <= 0.0 or depth_mm <= 0.0 or height_mm <= 0.0:
        raise ValueError("Cube width_mm, depth_mm, and height_mm must be greater than zero.")
    ...
    if obj is None:
        return _result("CANCELLED", "Blender did not create a cube object.")

    obj.scale = (
        mm_to_bu(width_mm, ctx),
        mm_to_bu(depth_mm, ctx),
        mm_to_bu(height_mm, ctx),
    )
    ensure_object_mode(ctx)
    make_active(obj, ctx)
    try:
    ...
    return _result(
        "FINISHED",
        f"Added Boolean-safe cube '{obj.name}'.",
        changed=1,
