...
def perform_boolsafe_cylinder(context=None, data=None):
    ctx = _ctx(context)
    ...
    if diameter_mm <= 0.0 or height_mm <= 0.0:
        raise ValueError("Cylinder diameter_mm and height_mm must be greater than zero.")
    if verts < 3:
        raise ValueError("Cylinder verts must be at least 3.")
    ...
    if obj is None:
        return _result("CANCELLED", "Blender did not create a cylinder object.")
