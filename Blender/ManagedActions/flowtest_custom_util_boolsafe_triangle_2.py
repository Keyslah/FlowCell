...
def perform_boolsafe_triangle(context=None, data=None):
    ctx = _ctx(context)
    side_mm = float(_data_value(data, "side_mm", DEFAULT_SIDE_MM))
    height_mm = float(_data_value(data, "height_mm", DEFAULT_HEIGHT_MM))

    if side_mm <= 0.0 or height_mm <= 0.0:
        raise ValueError("Triangle side_mm and height_mm must be greater than zero.")

    ensure_object_mode(ctx)
    bpy.ops.mesh.primitive_cylinder_add(
        radius=mm_to_bu(side_mm, ctx) / math.sqrt(3.0),
    ...
    if obj is None:
        return _result("CANCELLED", "Blender did not create a triangular prism object.")
