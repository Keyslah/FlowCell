...
function fileStem(path: string): string {
  ...
  return name.replace(/\.[^.]+$/, "").trim() || "Blender Theme";
}
...
export function registerBuiltinButtonCoreActions(): () => void {
  const unregister = [
    registerButtonCoreAction("open-illustrator-layer-tree", async (target) => {
    ...
    registerButtonCoreAction("open-window-grid", async (target) => {
      const programName = payloadString(target.payload, "programName");
      const panelName = payloadString(target.payload, "panelName");
      const fileName = payloadString(target.payload, "fileName");
      if (!programName || !panelName || !fileName) {
        throw new Error("The installed Window Grid Button is missing its owned source identity.");
      }
      await openWindowGridWindow();
      return { opened: true };
    }),
    registerButtonCoreAction("sample-blender-theme-image", async (_target, context) => {
      const imagePath = fieldString(context.fieldValues, "theme_image_path");
      if (!imagePath) throw new Error("Choose a theme image first.");
      const sampled = await invoke<SampledPhotoThemeColors>("sample_photo_theme_colors", {
        imagePath
      });
      return {
        fieldPatch: {
    ...
    registerButtonCoreAction("save-blender-theme-fields", async (_target, context) => {
